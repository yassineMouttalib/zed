...
fn open_sidebar_test_window(
    ...
    assert!(!projects.is_empty(), "need at least one project");
    ...
    let first_project = projects_iter.next().expect("at least one project");
    ...
    let sidebar = cx
        .update_window(multi_workspace_window.into(), |root_view, window, cx| {
            let mw_handle: Entity<MultiWorkspace> = root_view.downcast().unwrap();
            cx.new(|cx| sidebar::Sidebar::new(mw_handle, window, cx))
        })
        .context("Failed to create sidebar")?;
